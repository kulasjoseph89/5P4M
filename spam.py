...
:: : :    :             :::    :      :    
    ...
    contributors = "developer: "+" ".join(__CONTRIBUTORS__)
...
def do_zip_update():
    success = False
    if DEBUG_MODE:
        zip_url = "https://github.com/riz4d/5P4M/archive/main.zip"
        dir_name = "5P4M-main"
    ...
    if success:
        mesgdcrt.SuccessMessage("spam was updated to the latest version")
        mesgdcrt.GeneralMessage(
            "Please run the script again to load the latest version")
    else:
        mesgdcrt.FailureMessage("Unable to update spam.")
        mesgdcrt.WarningMessage(
            "Grab The Latest one From https://github.com/riz4d/5P4M.git")
...
def do_git_update():
    ...
    if success:
        mesgdcrt.SuccessMessage("5P4M was updated to the latest version")
        mesgdcrt.GeneralMessage(
            "Please run the script again to load the latest version")
    else:
        mesgdcrt.FailureMessage("Unable to update 5P4M.")
        mesgdcrt.WarningMessage("Make Sure To Install 'git' ")
        mesgdcrt.GeneralMessage("Then run command:")
        print(
            "git checkout . && "
            "git pull https://github.com/riz4d/5P4M.git HEAD")
    sys.exit()
...
def check_for_updates():
    ...
    fver = requests.get(
        "https://raw.githubusercontent.com/riz4d/5P4M/main/.version"
    ).text.strip()
    if fver != __VERSION__:
        ...
        mesgdcrt.SuccessMessage("5P4M is up-to-date")
        mesgdcrt.GeneralMessage("Starting 5P4M")


def notifyen():
    try:
        if DEBUG_MODE:
            url = "https://github.com/riz4d/5P4M/raw/main/.notify"
...
def pretty_print(cc, target, success, failed):
    ...
    mesgdcrt.SectionMessage("spamming is in progress - Please be patient")
    mesgdcrt.GeneralMessage(
    ...
    mesgdcrt.WarningMessage(
        "This tool was made for research purposes only")
    mesgdcrt.SuccessMessage("5P4M was created by R1Z4D")
...
def workernode(mode, cc, target, count, delay, max_threads):
    ...
    clr()
    mesgdcrt.SectionMessage("Gearing up the Spam - Please be patient")
    mesgdcrt.GeneralMessage(
        "Please stay connected to the internet during spamming")
    ...
    mesgdcrt.GeneralMessage("Delay         : " + str(delay) +
                            " seconds")
    mesgdcrt.WarningMessage(
        "This tool was made research purposes only")
    print()
    input(mesgdcrt.CommandMessage(
        "Press [CTRL+Z] to suspend the spam or [ENTER] to resume it"))
...
if sys.version_info[0] != 3:
    mesgdcrt.FailureMessage("5P4M will work only in Python v3")
    sys.exit()
...
description = """5P4M - Powerful Spammer Software

5P4M can be used for many purposes which incudes -
\t Exposing the vulnerable APIs over Internet
\t Friendly Spamming
\t Testing Your Spam Detector and more ....

5P4M is not intented for malicious uses.
"""

parser = argparse.ArgumentParser(description=description,
                                 epilog='Coded by rizad !!!')
parser.add_argument("-sms", "--sms", action="store_true",
                    help="start 5P4M with SMS 5P4M mode")
parser.add_argument("-call", "--call", action="store_true",
                    help="start 5P4M with CALL 5P4M mode")
parser.add_argument("-mail", "--mail", action="store_true",
                    help="start 5P4M with MAIL 5P4M mode")
parser.add_argument("-ascii", "--ascii", action="store_true",
                    help="show only characters of standard ASCII set")
parser.add_argument("-u", "--update", action="store_true",
                    help="update 5P4M")
parser.add_argument("-c", "--contributors", action="store_true",
                    help="show current 5P4M contributors")
parser.add_argument("-v", "--version", action="store_true",
                    help="show current 5P4M version")
...
if __name__ == "__main__":
    ...
    if args.version:
        ...
        print("developer: ", " ".join(__CONTRIBUTORS__))
    elif args.update:
        update()
    elif args.mail:
